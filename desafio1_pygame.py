...
def minimax(posicion_gato, posicion_raton, depth, maximizando, alpha=float('-inf'), beta=float('inf')):
    ...
    if maximizando:
        ...
        for mov in movimientos_validos(posicion_raton):
            ...
            # Valor de evaluacion de el posible movimiento actual (raton) contra el mejor proximo movimiento del gato
            eval, _ = minimax(posicion_gato, mov, depth - 1, False, alpha, beta)
            # Si la evaluacion actual es mayor (mejor) a nuestra max evaluacion anterior la reemplazamos
            if eval > max_eval:
                max_eval = eval
                mejor_movimiento = mov
            # Guarda el mayor valor entre si y eval
            alpha = max(alpha, eval)
            # Si el raton ya tiene una mejor jugada garantizada anterior podamos las evaluaciones (Optimizaicon)
            if beta <= alpha:
                break
...
# Ejecutamos el juego y definimos su profundidad (Profundidad maxima posible: 23)
